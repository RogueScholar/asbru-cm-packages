...
PACKAGE_VER="0.06"
ORIG_PACKAGE_NAME="perl-gtk2-unique-master"
PACKAGE_SRC="https://gitlab.gnome.org/GNOME/perl-gtk2-unique/-/archive/master/${ORIG_PACKAGE_NAME}.tar.gz"
...
# Download the module tarball from GitLab
echo "Downloading the official module source code from GitLab..."

case $TRANSFER_AGENT in
  wget)
    if wget -qc -t 3 --show-progress ${PACKAGE_SRC} -O "${PACKAGE_DIR}"/${PACKAGE_NAME}_${PACKAGE_VER}.orig.tar.gz; then
      echo -e '\t\e[37;42mOK:\e[0m Successfully downloaded the file from GitLab.'
    else
      echo -e "\\t\\e[37;41mERROR:\\e[0m Unable to download ${ORIG_PACKAGE_NAME} from GitLab."
      exit 1
    fi
    ;;
  curl)
...
# Unpack the module in a directory equivalent to its repository name and branch
